...
function UploadZone({ label, file, onClear, onChange }: UploadZoneProps) {
  return (
    <div className="relative group">
      <div
        className={`relative h-64 rounded-2xl border-2 border-dashed transition-all duration-300 flex flex-col items-center justify-center
           bg-white/2 p-6 ${
             file
               ? "border-violet-600/50 bg-voilet-500/5"
      ...
      >
        {file ? (
          <>
            ...
            />
            <div
              className="absolute inset-0 flex item-center justify-center 
          ...
          <>
            ...
            <p className="text-sm text-gray-400 text-center max-w-[200px]">
              Drag & drop or click to UploadIcon
            </p>
            <input
              type="file"
              accept="image/*"
